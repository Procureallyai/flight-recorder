# Devpost Browser Handoff

**Route:** Profile 7 trusted browser proof through the Codex Chrome Extension
**Submission:** `1100007-flight-recorder`
**Project:** `flight-recorder-jma65p`
**Current page:** Public submitted project page
**Current progress:** Five of five steps complete; submitted

## Project Details state

The verified project story from `DEVPOST_SUBMISSION.md`, the four accepted technology tags `OpenAI`, `Responses`, `Web`, and `Cryptography`, the public judge link, public repository link, final project thumbnail, two remediated gallery screenshots, and remediated public video address are saved. The screenshots exist as Devpost photo records `4937464` and `4937465`.

## Additional Information values

The following ordinary factual values are saved in the authenticated form:

- Submitter Type: `Individual`
- Category: `Developer Tools`
- Repository: `https://github.com/Procureallyai/flight-recorder`
- Judge project link and instructions:

  `Open https://flight-recorder-web.vercel.app/ in a modern browser. No account, credential, Application Programming Interface key, installation, or rebuild is required. Confirm the initial VERIFIED state, inspect Proof details and Accepted scope, select Alter covered artifact in memory, confirm INVALID and the artifact mismatch, then select Restore original artifact and confirm VERIFIED returns.`

- Developer-tool installation, supported platforms, and testing:

  `The public judge route requires only a modern browser and is the recommended no-build test path. Local capture was tested on macOS with desktop-bundled codex-cli 0.145.0-alpha.18. Local requirements are Node.js 22 or later and pnpm 8.15.9. From the repository root run pnpm install --frozen-lockfile, pnpm test, npm test --prefix demo/password-reset-workspace, and node packages/cli/dist/index.js verify fixtures/judge-passport/passport.json fixtures/judge-passport/artifacts --json.`

## Private field

- Country of Residence: `United Kingdom`, confirmed by Floyd Livingstone Rowe and selected in the authenticated form.
- `/feedback` Session Identifier: obtained from this primary Codex task, stored only in the gitignored local submission-secret file, and saved in the designated private Devpost field.
- Optional file upload: no file is required because the public repository, release, judge route, screenshots, and video provide the judging path.

## YouTube publication proof

Profile 7 trusted browser proof published the remediated film at `https://youtu.be/vOFVLWb5aM4` on 19 July 2026. The approved title, corrected description, final thumbnail, English (United Kingdom) language, Science and Technology category, not-made-for-children audience, and artificial-intelligence narration disclosure were saved. YouTube reported no copyright or Community Guidelines issues. In-app browser proof loaded the public watch page without authentication and showed the correct title, 2-minute 43-second player duration, and artificial-intelligence disclosure. Floyd confirmed playback acceptance of the remediated render on 19 July 2026.

## Submission confirmation

Floyd personally completed the final factual and legal review, accepted the applicable terms, and submitted the project. The Devpost connector records submission at 19 July 2026, 16:46:09 British Summer Time. Profile 7 trusted browser proof showed Devpost's `Project submitted!` confirmation at `https://devpost.com/software/flight-recorder-jma65p`.
