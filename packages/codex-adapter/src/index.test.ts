...
describe("codex exec JSON importer", () => {
  it("retains completed observable events and discards reasoning", () => {
    const result = importExecJsonLines([
      ...
      JSON.stringify({ type: "item.completed", item: { id: "file-1", type: "file_change", status: "completed", changes: [{ path: "src/reset.ts", kind: "update" }] } }),
      JSON.stringify({ type: "turn.completed", usage: { input_tokens: 100, output_tokens: 20 } }),
    ], { testedCodexVersion: "codex-cli 0.140.0", recordedAt: capturedAt });
  ...
  it("fails visibly on malformed input or a missing terminal event", () => {
    const result = importExecJsonLines(["not-json"], { testedCodexVersion: "codex-cli 0.140.0", recordedAt: capturedAt });
