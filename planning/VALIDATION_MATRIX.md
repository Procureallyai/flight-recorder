...
| Codex capture | Genuine observable session captured or supported fallback imported | Partial: version-pinned `codex exec --json` importer contract tests pass; installed `codex-cli 0.140.0` and App Server schemas were inspected. A genuine sanitised capture has not yet been imported. Live App Server handshake is route-blocked by a state backfill lock. |
...
| Quality | Formatting, lint, type check, unit, integration, end-to-end, and build gates pass | Partial: TypeScript build and 23 focused tests pass. Formatting, lint, browser end-to-end, and production application build are not yet implemented. |
